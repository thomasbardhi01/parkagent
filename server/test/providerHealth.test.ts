...
import { EXPIRY_WARNING_MS, earliestCookieExpiry, makeProviderHealth } from "../src/jobs/providerHealthTick.js";
...
function makeHealthHarness(options: {
  ...
  const health = makeProviderHealth({
    ...
    providerOps: () =>
      makeFakeProviderOps(options.verify ? { verifyAccount: options.verify } : {}),
    log: { info: () => {}, warn: () => {} },
    now: () => NOW,
  });
...
test("earliestCookieExpiry ignores session cookies without an expiry", () => {
  const state: ProviderStorageState = {
    cookies: [
      { name: "a", value: "", domain: "", path: "/", expires: -1, httpOnly: false, secure: true, sameSite: "Lax" },
      { name: "b", value: "", domain: "", path: "/", expires: 200, httpOnly: false, secure: true, sameSite: "Lax" },
      { name: "c", value: "", domain: "", path: "/", expires: 100, httpOnly: false, secure: true, sameSite: "Lax" },
    ],
    origins: [],
  };
...
test("an expiring account still pays: session start does not refuse it", async () => {
